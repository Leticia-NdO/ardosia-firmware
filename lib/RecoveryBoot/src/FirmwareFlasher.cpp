...
#include "RecoveryBootCompat.h"  // SPI_FLASH_SEC_SIZE + mbedTLS 2.x/3.x shim
...
uint16_t runningPartitionChipId() {
  ...
  static uint16_t cached = [] {
    ...
    // uint16_t local, so alignment is guaranteed on every MCU family.
...
namespace {
// Stream `length` bytes from `file` starting at the current read offset,
// feeding them through both the XOR-checksum and SHA256 accumulators. Used by
// validateImageFile so the whole image is verified end-to-end without holding
// it in RAM (the ESP32-C3 only has ~380 KB).
Result feedHashAndChecksum(FsFile& file, size_t length, uint8_t* xorAccum, mbedtls_sha256_context* sha,
                           uint8_t* buf) {
...
Result validateImageFile(const char* sdPath, size_t partitionSize) {
  ...
  if (file.read(header, HEADER_SIZE) != static_cast<int>(HEADER_SIZE)) {
    FLASH_LOG("validate: header read failed");
    file.close();
    return Result::READ_FAIL;
  }
  if (header[0] != ESP_IMAGE_MAGIC) {
    FLASH_LOG("validate: bad magic 0x%02X", header[0]);
    file.close();
  ...
  if (deviceChip != 0xFFFF && imageChip != deviceChip) {
    FLASH_LOG("validate: wrong chip: image=0x%04X device=0x%04X", imageChip, deviceChip);
    file.close();
  ...
  for (uint8_t i = 0; i < segCount; i++) {
    if (pos + SEG_HEADER_SIZE > fileSize) {
      FLASH_LOG("validate: seg %u header overruns EOF at %u", i, static_cast<unsigned>(pos));
    ...
    if (pos + dataLen > fileSize) {
      FLASH_LOG("validate: seg %u data overruns EOF (%u + %u > %u)", i, static_cast<unsigned>(pos),
  ...
  if (expectedTotal != fileSize) {
    FLASH_LOG("validate: size mismatch body+pad=%u sha=%u expected=%u actual=%u",
  ...
  if ((xorAccum & 0xFF) != storedChecksum) {
    FLASH_LOG("validate: checksum mismatch computed=0x%02X stored=0x%02X", xorAccum, storedChecksum);
    mbedtls_sha256_free(&shaCtx);
    file.close();
  ...
  if (hashAppended) {
    ...
    if (std::memcmp(computed, stored, SHA_TRAILER) != 0) {
      FLASH_LOG("validate: SHA256 mismatch");
      mbedtls_sha256_free(&shaCtx);
      file.close();
...
Result flashFromSdPath(const char* sdPath, ProgressCb onProgress, void* ctx, bool alreadyValidated) {
  // Resolve destination first so we can size-check during validation. The full
  // image-integrity pass verifies header, segment table, XOR checksum and
  // SHA256 trailer end-to-end before we touch otadata, so a truncated or
  // corrupted .bin can never become the next boot target.
  const esp_partition_t* dest = esp_ota_get_next_update_partition(nullptr);
  if (!dest) {
    FLASH_LOG("no next-update partition");
  ...
  if (!alreadyValidated) {
    ...
    if (validateRes != Result::OK) {
      FLASH_LOG("image validation failed: %s", resultName(validateRes));
      return validateRes;
    }
  }
  ...
  FsFile file;
  if (!SdMan.openFileForRead("FLASH", sdPath, file) || !file) {
    FLASH_LOG("open failed: %s", sdPath);
    return Result::OPEN_FAIL;
  }

  const size_t firmwareSize = static_cast<size_t>(file.size());
  FLASH_LOG("src=%s size=%u dest=%s @0x%x partsize=%u", sdPath, static_cast<unsigned>(firmwareSize),
  ...
  // Interleave erase + write so progress advances 0→100% smoothly rather than
  // stalling for several seconds during a single up-front erase.
  size_t streamPos = 0;
  size_t erasedUpto = 0;
  while (streamPos < firmwareSize) {
    if (streamPos >= erasedUpto) {
      ...
      if (esp_partition_erase_range(dest, streamPos, eraseLen) != ESP_OK) {
        FLASH_LOG("erase @%u (len=%u) failed", static_cast<unsigned>(streamPos),
    ...
    if (read <= 0 || static_cast<size_t>(read) != want) {
      FLASH_LOG("read @%u: got=%d want=%u", static_cast<unsigned>(streamPos), read,
                static_cast<unsigned>(want));
      file.close();
      return Result::READ_FAIL;
    }
    if (esp_partition_write(dest, streamPos, buffer.get(), want) != ESP_OK) {
      FLASH_LOG("write @%u failed", static_cast<unsigned>(streamPos));
      file.close();
  ...
  if (!recovery::switchBootPartition(dest)) {
    FLASH_LOG("otadata switch failed");
